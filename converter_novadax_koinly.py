...
def process_novadax_row(row):
    ...
    if "taxa de transacao" in tipo_normalizado:
        ...
        # Bônus que entra, mas sem atribuir o label "gift"
        received_amount = valor
        received_currency = moeda
        # Não atribuir o label "gift", mas garantir que o campo "label" não fique vazio
        label = ""  # Mantém vazio, ou se necessário, colocar "bonus" ou "deposit"
    ...
    # Se o label estiver vazio e for uma transação válida, podemos colocar algo como "deposit" ou "trade"
    if label == "":
        if "deposito" in tipo_normalizado:
            label = "deposit"
        elif "venda" in tipo_normalizado or "compra" in tipo_normalizado:
            label = "trade"
...
if __name__ == "__main__":
    # Coloque na linha abaixo o nome do seu arquivo .csv que quer converter
    ...
    # Para rodar, digite cole no terminal: python3 converter_novadax_koinly.py
